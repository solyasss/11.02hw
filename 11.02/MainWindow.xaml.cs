...
namespace hw
{
    public partial class MainWindow : Window
    {
        ...
        private void thread_second()
        {
            Dispatcher.Invoke(() =>
            {
                status_2.Text = "thread 2 waiting for mutex";
                progress_2.Value = 0;
            });

            mutex_file.WaitOne();

            Dispatcher.Invoke(() => { status_2.Text = "thread 2 is filtering prime numbers"; });
        ...
        private void thread_third()
        {
            Dispatcher.Invoke(() =>
            {
                status_3.Text = "thread 3 waiting for mutex";
                progress_3.Value = 0;
            });
            mutex_file.WaitOne();

            Dispatcher.Invoke(() => { status_3.Text = "thread 3 is filtering primes ending with 7"; });
            ...
            try
            {
                ...
                Dispatcher.Invoke(() =>
                {
                    status_3.Text = $"thread 3 found {primes7.Count} numbers ending with 7 in file";
                });
            }
            catch (Exception ex)
            {
